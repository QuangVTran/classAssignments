...
public class TranQuangFinal {
	...
	public static void main(String[] args) {
		...
		while(!aStack.isEmpty()) {
			aPriorityQueue.offer(aStack.pop());
		}
		
		while(!aQueue.isEmpty()) {
			aPriorityQueue.offer(aQueue.remove());
		}
		
		Node currentPtr = head;
		currentPtr.data = head.data;
		
		while(!aPriorityQueue.isEmpty()) {
			if(aPriorityQueue.peek() > 0) {
				currentPtr.next.data = aPriorityQueue.remove();
